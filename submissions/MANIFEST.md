...
|---|---|---|---|---|---|---|
| 56268283 | 2026-09-16 03:27 | `56268283__adarsh_single_repair.py` | `3177EFA6BB9F2D04` | 单张高分选手动作表 + V38 修复/护栏层 | **1784.9** | ★ **目前最佳** |
...
| pending | 2026-09-16 17:22 | `pending__adarsh_route_library.py` | `8AE5C0922B6133C2` | 11 张动作表按商店前缀自动选表 + 修复层 | ~1000（1h） | ✗ **不如单表版** |
...
| `pending__adarsh_route_library.tar.gz` | 202,836 | `453847C40ABE1660` |
...
4. **单表机制已被验证有效**（1784.9）。选表机制已被证伪（~1000）。
   失败原因是覆盖面：165 种商店前缀里库里只有 11 种，匹配不上时选表退化成「取列表第一张」，
   反而比那张精选的表更差。
